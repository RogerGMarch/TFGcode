...
//****************************************************************************
// Import movies from tree n_TOF raw2root movified for 3He experiment treeName const Char *
TH1D* GetMovieRaw2root_3He(string FileName="", const char* treeName=""){
    Short_t y;
    Float_t b;
    ...
    t1->SetBranchAddress("baseline",&b);
       ...
       Long64_t nentries = t1->GetEntries();
    TH1D * h   = new TH1D((treeName),(treeName),nentries,0,(Double_t) nentries);
    for (Long64_t i=0;i<nentries;i++) {
        t1->GetEntry(i);
        Double_t Sy=(Double_t) y;
        Double_t B=(Double_t) b;
...
//****************************************************************************
// Get a list of raw names from a root file of 3He experiment
...
std:: vector<std::__cxx11::string> RawNames(string FileName=""){
    ...
    std:: vector<std::__cxx11::string> Names_s;
...
TH1D* DerivativeWFM(TH1D* inputWFM, int StepSize=0, Double_t escala=1.0, Double_t diffOffset=0.0){
  ...
  //TH1D *WFM_derivative = ((TH1D *)(gROOT->FindObject("wfm_derivative"))); 
  //delete WFM_derivative;
...
TH1D* SmoothWFM(TH1D* inputWFM, int StepSize=0){
  int bins= (inputWFM-> GetNbinsX());
  //TH1D *SmoothWFM = ((TH1D *)(gROOT->FindObject("wfm_smooth"))); 
  //delete SmoothWFM;
...
double getProjectionThreshold(TH1D* inputWFM){
    ...
    TH1D *WFM_derivativePy = new TH1D("Py_wfm_derivative","Py_wfm_derivative",1000,-PyW,PyW);
